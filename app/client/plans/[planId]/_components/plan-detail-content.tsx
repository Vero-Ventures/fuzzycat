...
export function PlanDetailContent({ planId }: { planId: string }) {
  const trpc = useTRPC();
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(trpc.client.getPlanById.queryOptions({ planId }));
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <nav className="mb-6 flex items-center gap-1 text-sm text-muted-foreground">
        <Link href="/client/payments" className="hover:text-foreground">
          Dashboard
        </Link>
...
function OwnerPaymentSchedule({ planId }: { planId: string }) {
  ...
  const { data, isLoading, error } = useQuery(
    trpc.client.getPaymentHistory.queryOptions({ planId, page: 1, pageSize: 20 }),
  );
