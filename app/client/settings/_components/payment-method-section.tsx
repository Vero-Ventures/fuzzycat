...
export function PaymentMethodSection() {
  ...
  const { data: profile, isLoading } = useQuery(trpc.client.getProfile.queryOptions());
  const { data: paymentDetails, isLoading: isLoadingDetails } = useQuery({
    ...trpc.client.getPaymentMethodDetails.queryOptions(),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const invalidateQueries = () => {
    queryClient.invalidateQueries({ queryKey: trpc.client.getProfile.queryKey() });
    queryClient.invalidateQueries({ queryKey: trpc.client.getPaymentMethodDetails.queryKey() });
  };
  ...
  const setupCard = useMutation(
    trpc.client.setupCardPaymentMethod.mutationOptions({
      onSuccess: (data) => {
  ...
  const confirmCard = useMutation(
    trpc.client.confirmCardPaymentMethod.mutationOptions({
      onSuccess: () => {
        setSaveStatus('saved');
        invalidateQueries();
  ...
  const updateMutation = useMutation(
    trpc.client.updatePaymentMethod.mutationOptions({
      onSuccess: () => {
        setSaveStatus('saved');
        invalidateQueries();
        setTimeout(() => setSaveStatus('idle'), 2000);
  ...
  const removeMutation = useMutation(
    trpc.client.removePaymentMethod.mutationOptions({
      onSuccess: () => {
        setSaveStatus('saved');
        invalidateQueries();
        setTimeout(() => setSaveStatus('idle'), 2000);
  ...
  const createBankSetup = useMutation(
    trpc.client.createBankAccountSetupIntent.mutationOptions({
      onSuccess: (data) => {
  ...
  const confirmBank = useMutation(
    trpc.client.confirmBankAccount.mutationOptions({
      onSuccess: () => {
        setSaveStatus('saved');
        invalidateQueries();
        setTimeout(() => setSaveStatus('idle'), 2000);
