...
export function PetsSection() {
  ...
  const { data: petsList, isLoading } = useQuery(trpc.client.getPets.queryOptions());
  const [showAddForm, setShowAddForm] = useState(false);
  const [editingPetId, setEditingPetId] = useState<string | null>(null);

  const invalidatePets = () => {
    queryClient.invalidateQueries({ queryKey: trpc.client.getPets.queryKey() });
  };

  const addMutation = useMutation(
    trpc.client.addPet.mutationOptions({
      onSuccess: () => {
  ...
  const updateMutation = useMutation(
    trpc.client.updatePet.mutationOptions({
      onSuccess: () => {
  ...
  const removeMutation = useMutation(
    trpc.client.removePet.mutationOptions({
      onSuccess: () => {
        invalidatePets();
      },
    }),
