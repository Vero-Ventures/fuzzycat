...
const { provisionOwnerAccount } = await import('@/server/services/owner-provisioning');
...
describe('provisionOwnerAccount', () => {
  beforeEach(() => {
  ...
  describe('new owner (no auth account)', () => {
    ...
    it('creates Supabase auth account', async () => {
      ...
      await provisionOwnerAccount(BASE_PARAMS);
      ...
      expect(call.app_metadata).toEqual({ role: 'owner' });
    ...
    it('generates recovery link with correct redirect URL', async () => {
      ...
      expect(call.options.redirectTo).toContain(
        encodeURIComponent(`/owner/payments/${PLAN_ID}/deposit`),
      );
    });
    ...
    it('returns recovery link as setupUrl', async () => {
      ...
      const result = await provisionOwnerAccount(BASE_PARAMS);
  ...
  describe('existing owner (has auth account)', () => {
    ...
    it('returns direct deposit URL as setupUrl', async () => {
      ...
      expect(result.setupUrl).toBe(
        `https://test.fuzzycatapp.com/owner/payments/${PLAN_ID}/deposit`,
      );
    });
    ...
    it('still sends enrollment invite email', async () => {
      ...
      expect(props.setupUrl).toBe(`https://test.fuzzycatapp.com/owner/payments/${PLAN_ID}/deposit`);
    });
  });
